package com.example.sm.constants;

public class Constants {
    public static final String AEROSPIKE_SERVER_HOST_NAME="smaerospike";
    public static final int AEROSPIKE_SERVER_PORT=3000;
    public static final String AEROSPIKE_SERVER_NAMESPACE="test";
    public static final String BUNDLES_SOAP_URI="http://me:8083/SoapWS/bundles";
    public static final String SOAP_URI="http://me:8083/SoapWS";
    public static final String GET_BUNDLE_REQUEST_SOPA_ACTION="http://example.com/me/GetBundleRequest";
    public static final String ADD_BUNDLE_REQUEST_SOPA_ACTION="http://example.com/me/AddBundleRequest";
    public static final String DELETE_BUNDLE_REQUEST_SOPA_ACTION="http://example.com/me/DeleteBundleRequest";
    public static final String WSDL_CONTEXT_PATH="com.example.sm.wsdl";
}
